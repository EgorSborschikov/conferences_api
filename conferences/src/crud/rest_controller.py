...
from src.streaming.conference_state_management import active_conferences, active_streams, generate_unique_room_id
from src.streaming.signal_server import manager
...
async def joid_conference(room_id: str):
    if room_id not in active_conferences or not active_conferences[room_id].active:
        raise HTTPException(
            status_code = 404,
